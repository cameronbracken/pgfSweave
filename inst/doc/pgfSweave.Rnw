...
\subsubsection{The \code{tex.driver} option (Externalization Driver)}
...
Version 1.1.0 of \pkg{pgfSweave} introduced the \code{highlight} option.  The default is \code{FALSE} to maintain compatibility but if set to \code{TRUE} then echo'd code will be syntax-highlighted with the \pkg{highlight} package. For example:
