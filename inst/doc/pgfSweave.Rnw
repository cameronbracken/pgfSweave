...
\begin{Verbatim}[frame=single]
 ...
 syms <-c('alpha','theta','tau','beta','vartheta','pi','upsilon',
		  ...
		  'Theta','Pi','Phi')
 colors <- rep(c('red','blue','green'),length(syms))
 plot(0,type='n',xlim=c(-2*pi,2*pi),ylim=c(-1,1),axes=F,
 	    xlab='',ylab='',main='Feel the wave!')
 x <- seq(-2*pi,2*pi,length.out=length(syms))
 y <- sin(x)
 text(x,y,paste('\\color{',colors,'}$\\',syms,'$',sep=''))
 @  
\end{Verbatim}
