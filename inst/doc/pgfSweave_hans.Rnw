...
Here is a template \code{Sweave} document, note that \code{\textbackslash{}pgfrealjobname} should be the filename minus ``\code{.Rnw}'', so for this content to work, it should be saved as \code{my-title.Rnw}

\begin{Verbatim}[frame=single,label={my-title.Rnw},labelposition=topline]
 \documentclass{article}
 \usepackage{tikz}
 \pgfrealjobname{my-title}
...
For chunks with fig=F, dots are OK in the name:
...
The \code{pdf=T} is passed on to \code{tex2dvi}, I think is it there for tex to create pdf-files rather then encapsulated postcript files.
