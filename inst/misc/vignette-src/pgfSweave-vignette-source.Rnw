...
The option \code{tex.driver} not very well publicized, but it controls which engine (\code{latex}, \code{pdflatex}, \code{xelatex}, etc.) is used.  The value of this option gets passed to the command created in the shell script that is written. If the value is \code{latex} then an extra \code{dvips} command is issued to create an eps file. This way, the externalization feature can be used to generate eps and pdf external files. 
