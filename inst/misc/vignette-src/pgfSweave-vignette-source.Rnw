...
\SweaveOpts{prefix.string=figs/fig,fig=T,cache=T,tikz=T,external=T}
...
<<wave,echo=F,fig=F,tikz=F,external=F,results=tex,cache=F>>=
highlight('wave.R',renderer=renderer_latex(document=F))
@
}}

\begin{center}
<<fancy,echo=F,width=4,height=4>>=
source('wave.R')
@ 
\end{center}
...
\centering
<<normalSweave,echo=T,pdf=T,tikz=F,external=F,width=3,height=3>>=
hist(a)
@
...
\fbox{\parbox{\textwidth}{\small
<<thirdh,echo=F,fig=F,pgf=F,external=F,results=tex,cache=F>>=
highlight('third.R',renderer=renderer_latex(document=F))
@
}}
...
\begin{figure}[ht] 
\centering
<<third,echo=F,results=hide,width=5,height=5>>=
source('third.R')
@
