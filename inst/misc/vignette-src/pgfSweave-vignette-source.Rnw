...
That is because the vignette in \texttt{/inst/doc/} is a ``fake'' vignette generated from the ``real'' vignette in \texttt{/inst/misc/vignette-src/}.  The reason for this extra step is because compiling this vignette requires some from of the \pkg{pgfSweave} be installed.  The other problem is that package vignettes must be able to be compiled with \texttt{R CMD Sweave <vignette>.Rnw}, which is just what we don't want to use! Sort of a chicken or the egg problem.  

To compile this vignette yourself use the following:

\begin{verbatim}
$ svn checkout svn://svn.rforge.net/pgfSweave/trunk pgfSweave
